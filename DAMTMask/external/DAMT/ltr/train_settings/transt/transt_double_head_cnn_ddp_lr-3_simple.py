...
def run(settings):
    ...
    # Transformer
    settings.use_transformer = False
    ...
    settings.transt_path = '/home/tiger/tracking_code/transt_mask_cvt/models/pretrained_TransT_M2.pth.tar'
    # settings.transt_path = None

    # loss
    settings.loss_mask_weight = 0
    settings.loss_dice_weight = 0
    settings.loss_bce_weight = 1000
    ...
    # Optimizer
    optimizer = torch.optim.AdamW(model_without_ddp.parameters(), lr=1e-3)
    ...
    trainer.train(120, transt_path=settings.transt_path, load_latest=True, fail_safe=True, excluded_key = None)
