...
import ltr.models.segm_res.res_init as res_models
...
def run(settings):
    ...
    settings.template_feature_sz = 32
    settings.search_sz = settings.search_feature_sz * 8
    settings.temp_sz = settings.template_feature_sz * 8
    settings.center_jitter_factor = {'search': 1.5, 'static_template': 0.25, 'dynamic_template': 0.25}
    ...
    settings.transt_path = '/home/tiger/tracking_code/TransT_M/models/TransTiouhsegm_ep0090.pth.tar'
    ...
    dataset_train = sampler.TransTMaskSampler([youtube_vos,saliency,ovis,youtube_vis], [2,3,2,2],
                                              ...
                                              samples_per_epoch=32*200*settings.batch_size, max_gap=100,
    ...
    #model = res_models.transt_resnet50(settings)
    model = res_models.transt_resnet50(settings)
    ...
    objective = res_models.transt_loss(settings)

    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('number of params:', n_parameters)

    actor = actors.TranstActorSegmFusion(net=model, objective=objective, settings=settings)
    ...
    trainer.train(120, transt_path=settings.transt_path, load_latest=True, fail_safe=True, excluded_key = 'mask_head')
