...
def sample_target_with_pos_and_mask(im,abspos,mask, target_bb, search_area_factor, output_sz=None, feature_sz=None):
    ...
    if any([top_pad, bottom_pad, left_pad, right_pad]):
        ...
        size_abspos = (r + top_pad + bottom_pad, c + left_pad + right_pad, k_abspos)
        te_abspos = np.zeros(size_abspos, np.float32)
        te_abspos[top_pad:top_pad + r, left_pad:left_pad + c, :] = abspos

        size_mask = (r + top_pad + bottom_pad, c + left_pad + right_pad, k_mask)
        te_mask = np.ones(size_mask, np.float32)
        te_mask[top_pad:top_pad + r, left_pad:left_pad + c, :] = mask

        if top_pad:
            te_im[0:top_pad, left_pad:left_pad + c, :] = avg_chans
            te_abspos[0:top_pad, left_pad:left_pad + c, :] = 0
            te_mask[0:top_pad, left_pad:left_pad + c, :] = 1
        if bottom_pad:
            te_im[r + top_pad:, left_pad:left_pad + c, :] = avg_chans
            te_abspos[r + top_pad:, left_pad:left_pad + c, :] = 0
            te_mask[r + top_pad:, left_pad:left_pad + c, :] = 1
        if left_pad:
            te_im[:, 0:left_pad, :] = avg_chans
            te_abspos[:, 0:left_pad, :] = 0
            te_mask[:, 0:left_pad, :] = 1
        if right_pad:
            te_im[:, c + left_pad:, :] = avg_chans
            te_abspos[:, c + left_pad:, :] = 0
            te_mask[:, c + left_pad:, :] = 1
        im_patch = te_im[int(context_ymin):int(context_ymax + 1),
                   int(context_xmin):int(context_xmax + 1), :]

        abspos_patch = te_abspos[int(context_ymin):int(context_ymax + 1),
                   int(context_xmin):int(context_xmax + 1), :]
        ...
        im_patch = im[int(context_ymin):int(context_ymax + 1),
                   int(context_xmin):int(context_xmax + 1), :]
        abspos_patch = abspos[int(context_ymin):int(context_ymax + 1),
                   int(context_xmin):int(context_xmax + 1), :]
    ...
    if not np.array_equal(feature_sz,crop_sz):
        abspos_patch = cv.resize(abspos_patch, (feature_sz, feature_sz))
        mask_patch = cv.resize(mask_patch, (feature_sz, feature_sz))

    resize_factor = output_sz / crop_sz
    if len(im_patch.shape) == 2:
        im_patch = im_patch[..., np.newaxis]
    return im_patch,abspos_patch,mask_patch,resize_factor
...
def jittered_center_crop_with_position(frames, max_coord_map, paddingmask, box_extract, box_gt, search_area_factor, output_sz, feature_sz):
    """ For each frame in frames, extracts a square crop centered at box_extract, of area search_area_factor^2
    times box_extract area. The extracted crops are then resized to output_sz. Further, the co-ordinates of the box
    box_gt are transformed to the image crop co-ordinates
    ...
    img_crops_resize_factors = [sample_target_with_pos_and_mask(f,coord,pm[..., np.newaxis], a, search_area_factor, output_sz, feature_sz)
                            for f,coord,pm,a in zip(frames,max_coord_map,paddingmask, box_extract)]

    frames_crop,coordmaps_crop,paddingmasks_crop, resize_factors = zip(*img_crops_resize_factors)
    ...
    return frames_crop, box_crop, list(coordmaps_crop), list(paddingmasks_crop)
...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
    ...
    center = feat_sz * target_center_norm
