...
class Mask_Predictor_fine(nn.Module):
    ...
    def forward(self, corr_feat, Lfeat):
        ...
        '''corr_feat: (batch,64,16,16)
           Lfeat: (batch,64,128,128), (batch,256,64,64), (batch,512,32,32)'''
        sz = corr_feat.shape[-1]
        out = self.post0(F.interpolate(self.h2(corr_feat), size=(sz << 1, sz << 1), mode='bilinear') + self.v2(
            Lfeat[2]))  # (b,32,32,32)+(b,32,32,32) --> (b,16,32,32)
        out = self.post1(F.interpolate(self.h1(out), size=(sz << 2, sz << 2), mode='bilinear') + self.v1(
            Lfeat[1]))  # (b,16,64,64)+(b,16,64,64) --> (b,4,64,64)
        out = self.post2(
            (F.interpolate(self.h0(out), size=(sz << 3, sz << 3), mode='bilinear') + self.v0(Lfeat[0])).contiguous())
        out = torch.sigmoid(F.interpolate(out, size=(sz << 4, sz << 4), mode='bilinear'))  # (b,1,256,256)
        return out
