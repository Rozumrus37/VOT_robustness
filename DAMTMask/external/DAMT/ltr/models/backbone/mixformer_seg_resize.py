...
from .configs import mixformer_cfg
...
# update config
# cfg = mixformer_cfg.cfg
# exp_config = os.path.join(os.path.dirname(__file__) + '/configs/baseline_1k.yaml')
# mixformer_cfg.update_config_from_file(exp_config)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, template, online_template, search, template_seg, online_template_seg):
        """
        ...
        template_seg = torch.nn.functional.interpolate(template_seg,size = list(template.shape[-2:]))
        online_template_seg = torch.nn.functional.interpolate(online_template_seg,size = list(template.shape[-2:]))
        template_seg = self.mask_embed(template_seg)
        online_template_seg = self.mask_embed(online_template_seg)

        template = self.patch_embed(template) + template_seg
        online_template = self.patch_embed(online_template) + online_template_seg
...
class ConvolutionalVisionTransformer(nn.Module):
    ...
    def forward(self, template, online_template, search, template_seg, online_template_seg):
        ...
        """
        # 20220412
        templates, online_templates, searchs = dict(), dict(), dict()
        # template = template + self.template_emb
        # search = search + self.search_emb
        for i in range(self.num_stages):
            template, online_template, search = getattr(self, f'stage{i}')(template, online_template, search, template_seg, online_template_seg)
            templates[f'stage{i}'] = template
            online_templates[f'stage{i}'] = online_template
            searchs[f'stage{i}'] = search

        #return template, search
        return templates, online_templates, searchs
...
def get_mixformer_model(use_large_backbone = False, **kwargs):
    if use_large_backbone:
        exp_config = os.path.join(os.path.dirname(__file__) + '/configs/baseline_large.yaml')
    else:
        exp_config = os.path.join(os.path.dirname(__file__) + '/configs/baseline_1k.yaml')
    mixformer_cfg.update_config_from_file(exp_config)
    config = mixformer_cfg.cfg
...
if __name__ == '__main__':
    ...
    dummy_template = torch.Tensor(1,3,128,128)
    dummy_search = torch.Tensor(1,3,320,320)

    print(f"Input:\ntemplate image: {dummy_template.shape}, search image: {dummy_search.shape}")
    f_template, f_search = backbone(dummy_template, dummy_template, dummy_search)
    print(f"Output:\ntemplate feature: {f_template.shape}, search feature: {f_search.shape}")
