...
class FeatureFusionSegNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
                 dim_feedforward=2048, dropout=0.1,downrate = 8, activation="relu"):
        super().__init__()

        stem_layer = Focus(out_channels = d_model,downrate = downrate)

        featurefusion_layer = FeatureFusionLayer(d_model, nhead, dim_feedforward, dropout, activation)
        self.encoder = Encoder(featurefusion_layer,stem_layer, num_featurefusion_layers)
    ...
    def forward(self, src_temp, mask_temp, pos_temp,seg_temp, src_search, mask_search, pos_search):

        src_temp = src_temp.flatten(2).permute(2, 0, 1).contiguous()
        pos_temp = pos_temp.flatten(2).permute(2, 0, 1).contiguous()

        search_shape = src_search.shape #torch.Size([16, 256, 32, 32])
        src_search = src_search.flatten(2).permute(2, 0, 1).contiguous()
        pos_search = pos_search.flatten(2).permute(2, 0, 1).contiguous()

        mask_temp = mask_temp.flatten(1).contiguous()
        mask_search = mask_search.flatten(1).contiguous()
        ...
        memory = hs.permute(1, 2, 0).reshape(search_shape).contiguous()
        hs = hs.unsqueeze(0).transpose(1, 2).contiguous()
        hs_temp = memory_temp.unsqueeze(0).transpose(1, 2).contiguous()
        return hs, hs_temp, memory
...
class Encoder(nn.Module):
    ...
    def __init__(self, featurefusion_layer,stem_layer, num_layers):
        super().__init__()
        self.layers = _get_clones(featurefusion_layer, num_layers)
        self.stem_layers = _get_clones(stem_layer, num_layers)
        self.num_layers = num_layers

    def forward(self, src1, src2, seg1,
        ...
        for i in range(self.num_layers):
            output1 = output1 + self.stem_layers[i](seg1).flatten(2).permute(2, 0, 1)

            output1, output2 = self.layers[i](output1, output2, src1_mask=src1_mask,
                                     src2_mask=src2_mask,
...
def build_featurefusion_seg_network(settings):
    return FeatureFusionSegNetwork(
        ...
        downrate = getattr(settings, "downrate", 8)
    )
