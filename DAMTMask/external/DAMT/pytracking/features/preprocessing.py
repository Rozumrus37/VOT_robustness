...
def sample_patch(im: torch.Tensor, pos: torch.Tensor, sample_sz: torch.Tensor, output_sz: torch.Tensor = None,
    ...
    if not is_mask:
        ...
        im_patch = F.pad(im2, (int(-tl[1].item()), int(br[1].item()) - im2.shape[3], int(-tl[0].item()), int(br[0].item()) - im2.shape[2]), mode)

    else:
        #im_patch = F.pad(im2, (-tl[1].item(), br[1].item() - im2.shape[3], -tl[0].item(), br[0].item() - im2.shape[2]))
        im_patch = F.pad(im2, (int(-tl[1].item()), int(br[1].item()) - im2.shape[3], int(-tl[0].item()), int(br[0].item()) - im2.shape[2]))
