...
from ltr.data.processing_utils_SE import (sample_target_SE, sample_target_SE_with_mask,
                                          transform_image_to_crop_SE, map_mask_back)
...
class RefineModule(object):
    ...
    def initialize(self, frame1, bbox1, mask1):
        """
        Args:
        ...
        patch1, p_mask1, h_f, w_f = sample_target_SE_with_mask(frame1, bbox1, mask1, self.search_factor, self.input_sz, mode=cv2.BORDER_CONSTANT)
        patch1_tensor = self.img_preprocess(patch1)
        p_mask1_tensor = self.mask_preprocess(p_mask1)
        ...
        with torch.no_grad():
            self.m_key, self.m_value = self.refine_network.forward_ref(patch1_tensor, p_mask1_tensor)
    ...
    def refine(self, Cframe, Cbbox, mode='all', test=False):
        ...
        #===========show
        self.vis_img_mask = Cpatch.copy()
        self.vis_mask_scale = [h_f,w_f]
        #===========show
        ...
        with torch.no_grad():
            ...
            output = self.refine_network.forward_test(Cpatch_tensor, self.m_key, self.m_value,
                                                      mode='test', branches=['corner', 'mask', ])  # (1,1,H,W)
            ...
            if mode == 'bbox' or mode == 'corner':
                ...
                if 'corner' in output:
                    ...
                    #===========show
                    self.vis_mask_box = Pbbox_arr.copy()
                    #===========show
                ...
                if 'mask' in output:
                    ...
                    #===========show
                    self.vis_mask_raw = Pmask_arr.copy()
                    #===========show
                    output_dict['mask'] = map_mask_back(Cframe, Cbbox, self.search_factor, Pmask_arr,
                                                        mode=cv2.BORDER_CONSTANT)
        ...
        # print(time.time() - tic)
        # #===========show
        # self.vis_mask_corr_weight_softmax_topk = self.refine_network.neck.show_weight_softmax_topk
        # self.vis_mask_corr_weight_softmax = self.refine_network.neck.show_weight_softmax
        # self.vis_mask_corr_weight = self.refine_network.neck.show_weight
        # #===========show
    ...
    def mask_preprocess(self, mask):
        ...
        Return:
            (1,1,1,H,W)
        """
        img_tensor = torch.from_numpy(mask.astype(np.float32)).cuda()
        img_tensor = img_tensor.permute((2, 0, 1))
        return img_tensor.unsqueeze(dim=0).unsqueeze(dim=0)
