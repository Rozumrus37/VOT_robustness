...
class TRANST_CVT_SEG(object):
    ...
    def initialize(self, image, mask):
        ...
        template = self.net.template_cvt(z_crop,z_mask)
    ...
    def track(self, image, bbox):
        ...
        # track
        outputs = self.net.get_mask(x_crop, self.template['src_temp'], self.template['mask'])

        pred_mask_search = self._convert_mask(outputs['pred_masks'])
        self.vis_transtcvt_maskraw = cv2.resize(pred_mask_search.copy(),(self.instance_size,self.instance_size))

        pred_mask = self.map_mask_back(image, self.center_pos, self.instance_size, s_x, pred_mask_search,
                                  mode=cv2.BORDER_CONSTANT)

        self.final_mask_raw = pred_mask
