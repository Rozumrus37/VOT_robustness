...
def parse_args():
    ...
    parser.add_argument('--mixformer_model', type=str, default=None, help='stage1 model used to train SPM.')
...
def main():
    ...
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--script_prv %s --config_prv %s --distill %d --script_teacher %s --config_teacher %s --stage1_model %s --mixformer_model %s" \
                    % (args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv,
                       args.distill, args.script_teacher, args.config_teacher, args.stage1_model, args.stage1_model, args.mixformer_model)
        ...
        train_cmd = "python -m torch.distributed.launch --master_port %d --nproc_per_node %d lib/train/run_training.py " \
                    ...
                    "--distill %d --script_teacher %s --config_teacher %s --stage1_model %s --mixformer_model %s" \
                    % (args.master_port, args.nproc_per_node, args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv,
                       args.config_prv, args.distill, args.script_teacher, args.config_teacher, args.stage1_model, args.mixformer_model)
    else:
