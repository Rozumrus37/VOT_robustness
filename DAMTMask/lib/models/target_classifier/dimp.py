...
from lib.models.meta import steepestdescent
import lib.models.target_classifier.linear_filter as target_clf
import lib.models.target_classifier.features as clf_features
import lib.models.target_classifier.initializer as clf_initializer
import lib.models.target_classifier.optimizer as clf_optimizer
from lib.models.target_classifier import residual_modules
...
class DiMPnet(nn.Module):
    ...
    def __init__(self, classifier,dimp_merge_feature_layer):
        super().__init__()

        # self.feature_extractor = feature_extractor
        self.classifier = classifier
        self.dimp_merge_feature_layer = dimp_merge_feature_layer
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_bb, *args, **kwargs):
        ...
        #train_imgs = [num_images,bs,c,h,w]
        #train_bb = [num_images,bs,4]
        ...
        target_scores = self.classifier(train_imgs, test_imgs, train_bb, test_bb, *args, **kwargs)
...
def dimpnet50(settings):

    '''
    filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              feat_stride=16, clf_feat_blocks=0,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True,
    ...
    # Feature normalization
    norm_scale = math.sqrt(1.0 / (settings.out_feature_dim * settings.filter_size * settings.filter_size))

    # Merge features
    dimp_merge_feature_layer = merge_features.Merge(inchannels = [settings.backbone_outdim[i] for i in settings.merge_layer],
                                                outchannel = settings.backbone_outdim[settings.merge_layer[-1]],
                                                midchannel = settings.backbone_outdim[settings.merge_layer[-1]])


    # Classifier features
    # backbone_feature_dim = settings.backbone_outdim[-1]//4
    backbone_feature_dim = settings.backbone_outdim[settings.merge_layer[-1]]//4
    clf_feature_extractor = clf_features.residual_bottleneck(feature_dim=backbone_feature_dim,
                                                             num_blocks=settings.clf_feat_blocks, l2norm=settings.clf_feat_norm,
                                                             final_conv=settings.final_conv, norm_scale=norm_scale,
                                                             out_dim=settings.out_feature_dim)

    # Initializer for the DiMP classifier
    initializer = clf_initializer.FilterInitializerLinear(filter_size=settings.filter_size, filter_norm=settings.init_filter_norm,
                                                          feature_dim=settings.out_feature_dim)

    # Optimizer for the DiMP classifier
    optimizer = clf_optimizer.DiMPSteepestDescentGN(num_iter=settings.optim_iter, feat_stride=settings.feat_stride,
                                                    init_step_length=settings.optim_init_step,
                                                    init_filter_reg=settings.optim_init_reg, init_gauss_sigma=settings.init_gauss_sigma,
                                                    num_dist_bins=settings.num_dist_bins,
                                                    bin_displacement=settings.bin_displacement,
                                                    mask_init_factor=settings.mask_init_factor,
                                                    score_act=settings.score_act, act_param=settings.act_param, mask_act=settings.target_mask_act,
                                                    detach_length=settings.detach_length)

    # The classifier module
    classifier = target_clf.LinearFilter(filter_size=settings.filter_size,
                                         filter_initializer=initializer,
                                         filter_optimizer=optimizer, feature_extractor=clf_feature_extractor,
                                         robust_filter = settings.robust_filter, drop_prob = settings.drop_prob)

    # # Bounding box regressor
    # bb_regressor = bbmodels.AtomIoUNet(input_dim=(4*128,4*256), pred_input_dim=iou_input_dim, pred_inter_dim=iou_inter_dim)

    # DiMP network
    net = DiMPnet(classifier=classifier,dimp_merge_feature_layer = dimp_merge_feature_layer)
    return net
...
def dimpnet50_simple(filter_size=1, optim_iter=5, optim_init_reg=0.01,
    ...
    # Feature normalization
    norm_scale = math.sqrt(1.0 / (cfg.out_feature_dim * cfg.filter_size * cfg.filter_size))


    # Classifier features
    clf_feature_extractor = clf_features.residual_bottleneck(feature_dim=cfg.feature_dim,
                                                             num_blocks=cfg.clf_feat_blocks, l2norm=cfg.clf_feat_norm,
                                                             final_conv=cfg.final_conv, norm_scale=norm_scale,
                                                             out_dim=cfg.out_feature_dim)

    # Initializer for the DiMP classifier
    initializer = clf_initializer.FilterInitializerLinear(filter_size=cfg.filter_size, filter_norm=cfg.init_filter_norm,
                                                          feature_dim=cfg.out_feature_dim)
...
def klcedimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    # DiMP network
    net = DiMPnet(classifier=classifier)
    return net
