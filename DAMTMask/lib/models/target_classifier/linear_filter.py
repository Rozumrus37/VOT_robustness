...
import lib.models.layers.filter as filter_layer
import math
...
class LinearFilter(nn.Module):
    ...
    def __init__(self, filter_size, filter_initializer, filter_optimizer=None, feature_extractor=None, robust_filter = False, drop_prob = None):
        super().__init__()
    ...
    def forward(self, train_feat, test_feat, train_bb, test_bb, *args, **kwargs):
        ...
        assert train_bb.shape[1] == test_bb.shape[1]
        ...
        if self.robust_filter:
            filter, filter_iter, losses = self.get_filter_robust(train_feat, train_bb, test_feat, test_bb, True, *args, **kwargs)
        else:
    ...
    def get_filter_robust(self, train_feat, train_bb, test_feat, test_bb, training = True, *args, **kwargs):
        ...
        weights = self.filter_initializer(train_feat, train_bb)

        if training:
            test_feat_drop = F.dropout2d(test_feat, p=self.drop_prob, training=True)
        else:
            test_feat_drop = test_feat

        if self.filter_optimizer is not None:
            weights, weights_iter, losses = self.filter_optimizer(weights, feat=test_feat_drop, bb=test_bb, *args, **kwargs)
        else:
