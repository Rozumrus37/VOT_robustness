...
from lib.models.target_classifier import dimp
from lib.models.head.reppoint.reppoints_head import RepPointsHead
...
class ConvolutionalVisionTransformer(nn.Module):
    ...
    def forward(self, template, online_template, search):
        ...
        template_list = []
        online_template_list = []
        search_list = []
        for i in range(self.num_stages):
            template, online_template, search = getattr(self, f'stage{i}')(template, online_template, search)
            template_list.append(template)
            online_template_list.append(online_template)
            search_list.append(search)
        return template_list,online_template_list, search_list
    ...
    def forward_test(self, search):
        ...
        for i in range(self.num_stages):
            search = getattr(self, f'stage{i}').forward_test(search)
            search_list.append(search)
        return self.template, search_list
...
class MixFormerOnlineScore(nn.Module):
    ...
    def __init__(self, backbone, box_head, dimp_branch = None,reppoint_head = None, score_branch=None, head_type="CORNER"):
        """ Initializes the model.
        """
        super().__init__()
        self.backbone = backbone
        self.box_head = box_head
        self.dimp_branch = dimp_branch
        self.reppoint_head = reppoint_head
        self.score_branch = score_branch
        self.head_type = head_type

    def forward(self, template, online_template, search, run_score_head=True, template_bboxes = None, search_bboxes=None):
        # search: (b, c, h, w)
        # search_bboxes 预测score 需要做归一化
        # template_bboxes 没有归一化的bbox
        size_search = search.shape

        if template.dim() == 5:
            num_img,bs,c,h,w = template.shape
            template = template.reshape(num_img*bs,c,h,w)

        if online_template.dim() == 5:
            num_img,bs,c,h,w = online_template.shape
            online_template = online_template.reshape(num_img*bs,c,h,w)

        if search.dim() == 5:
            num_img,bs,c,h,w = search.shape
            search = search.reshape(num_img*bs,c,h,w)

        # if search_bboxes.dim() == 3:
        #     num_img,bs,_ = search_bboxes.shape
        #     search_bboxes = search_bboxes.reshape(num_img*bs, 4)

        template_list,online_template_list, search_list = self.backbone(template, online_template, search)
        # search shape: (b, 384, 20, 20)
        # Forward the corner head and score head
        #==========================corner
        out, outputs_coord_new = self.forward_head(search_list[-1], template_list[-1], run_score_head, search_bboxes.clone(), size_search[-1])
        #==========================corner

        #==========================dimp
        dimp_out = self.forward_dimp(template_list, online_template_list, search_list, template_bboxes.clone(), search_bboxes.clone(),batch_size = bs)
        out.update(dimp_out)
        #==========================dimp

        #==========================reppoint
        reppoint_out = self.forward_reppoint(search_list)
        out.update(reppoint_out)
        #==========================reppoint
    ...
    def forward_reppoint(self,search):
        #searche [num_img*bs,c,h,w]
        search = self.reppoint_head.reppoint_merge_feature_layer(search)
        cls_out,pts_out_init,pts_out_refine = self.reppoint_head(search)
        out = {'reppoint_cls': cls_out,
                'reppoint_init_bboxes': pts_out_init,
                'reppoint_refine_bboxes': pts_out_refine,
                }
        return out


    def extract_dimp_feat(self,template,search):
        #searche [num_img*bs,c,h,w]
        #templates [num_img*bs,c,h,w]
        #gt_bboxes [2*num_img,bs,4]
        search = self.dimp_branch.dimp_merge_feature_layer(search)
        template = self.dimp_branch.dimp_merge_feature_layer(template)
        # online_template = self.dimp_branch.dimp_merge_feature_layer(online_template)

        templates = template
        searches = search

        templates = self.dimp_branch.classifier.extract_classification_feat(templates)
        searches = self.dimp_branch.classifier.extract_classification_feat(searches)

        return templates, searches

    def forward_dimp(self,template,online_template,search, template_bboxes, search_bboxes, batch_size):
        #searche [num_img*bs,c,h,w]
        #templates [num_img*bs,c,h,w]
        #gt_bboxes [2*num_img,bs,4]
        ...
        template = template.reshape([-1,batch_size] + list(template.shape[-3:]))
        online_template = online_template.reshape([-1,batch_size] + list(online_template.shape[-3:]))
        search = search.reshape([-1,batch_size] + list(search.shape[-3:]))

        templates = torch.cat([template,online_template],0)
        searches = search

        target_scores = self.dimp_branch(templates, searches, template_bboxes.clone(), search_bboxes.clone())

        out = {'dimp_scores': target_scores}
        return out

    def forward_head(self, search, template, run_score_head=True, gt_bboxes=None, size = None):
        """
        ...
        if run_score_head:
            ...
            if gt_bboxes is None:
                ...
                gt_bboxes = box_xywh_to_xyxy(gt_bboxes.clone().view(-1,4)) / size #归一化
            # (b,c,h,w) --> (b,h,w)
...
def build_mixformer_online_score_dimp_reppoint(cfg, settings=None, train=True):
    ...
    dimp_branch = dimp.dimpnet50(settings.dimp_param)
    reppoint_head = RepPointsHead(settings.reppoint_param)

    model = MixFormerOnlineScore(
        backbone = backbone,
        box_head = box_head,
        dimp_branch = dimp_branch,
        reppoint_head = reppoint_head,
        score_branch= score_branch,
        head_type=cfg.MODEL.HEAD_TYPE
    )

    if cfg.MODEL.PRETRAINED_MIXFORMER and train:
        try:
            ckpt_path = settings.mixformer_model
            ckpt = torch.load(ckpt_path, map_location='cpu')
            missing_keys, unexpected_keys = model.load_state_dict(ckpt['net'], strict=False)
            if is_main_process():
                print("missing keys:", missing_keys)
                print("unexpected keys:", unexpected_keys)
                print("Loading pretrained mixformer weights done.{}".format(ckpt_path))
        except:
