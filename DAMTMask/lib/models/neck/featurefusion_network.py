...
"""
NECK FeatureFusionNetwork class.
...
class FeatureFusionNetwork(nn.Module):
    ...
    def forward(self, src_temp, src_search, pos_temp, pos_search):
        mask_temp = torch.zeros((src_temp.shape[0], *src_temp.shape[-2:]), dtype=torch.bool, device=src_temp.device)
        mask_search = torch.zeros((src_search.shape[0], *src_search.shape[-2:]), dtype=torch.bool, device=src_search.device)

        search_shape = src_search.shape
        template_shape = src_temp.shape
        ...
        pos_search = pos_search.flatten(2).permute(2, 0, 1) #h*w,b,c,
        ...
        memory_temp = memory_temp.permute(1, 2, 0).reshape(template_shape)
        memory_search = memory_search.permute(1, 2, 0).reshape(search_shape)

        hs_search_mlp = hs.transpose(0, 1).contiguous()
        hs_search_cnn = hs.permute(1, 2, 0).reshape(search_shape)

        return memory_temp,memory_search,hs_search_mlp,hs_search_cnn
...
def build_featurefusion_network(cfg):
    return FeatureFusionNetwork(
        d_model=cfg.MODEL.NECK.HIDDEN_DIM,
        dropout=cfg.MODEL.NECK.DROPOUT,
        nhead=cfg.MODEL.NECK.NHEADS,
        dim_feedforward=cfg.MODEL.NECK.DIM_FEEDFORWARD,
        num_featurefusion_layers=cfg.MODEL.NECK.NUM_FEATUREFUSION_LAYERS
    )
