...
class MixFormerKLDimpDFcosTranstActor(BaseActor):
    ...
    def __call__(self, data, is_train = True):
        ...
        # compute losses
        loss, status = self.compute_losses_dimp_fcos_cls_reg(out_dict, data)
    ...
    def compute_losses_dimp_fcos_cls_reg(self, pred_dict, data, return_status=True):
        pred_cls = pred_dict['pred_cls']
        if torch.isnan(pred_cls).any():
            raise ValueError("Network outputs is NAN! Stop Training")
        pred_reg = pred_dict['pred_reg']
        if torch.isnan(pred_reg).any():
            raise ValueError("Network outputs is NAN! Stop Training")
        ...
        #==============================transt
        losses = self.objective['transt'](pred_dict, data)
        loss_transt = losses['loss_cls'] * self.loss_weight['cls'] \
                       + losses['loss_reg_giou'] * self.loss_weight['reg_giou'] \
                       + losses['loss_reg_l1'] * self.loss_weight['reg_l1']
        loss += loss_transt
         #==============================transt
        ...
        status = {"Loss/total": loss.item(),
                      ...
                      "Loss/transt": loss_transt.item(),

                      "iou_fc": iou_fc.item(),
                      "iou_conv": iou_conv.item(),
                      "iou_corner": mean_iou_corner.item(),
                      "iou_transt": losses['iou'].item(),
                      }
        if 'test_clf' in self.loss_weight.keys():
