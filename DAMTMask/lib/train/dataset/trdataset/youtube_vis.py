import os
from lib.train.dataset.trdataset.base_dataset import BaseDataset
...
class YoutubeVIS2021(BaseDataset):
    ...
    def __init__(self, split='train', root=None, image_loader=default_image_loader, data_fraction=None):
        ...
        args:
            ...
            data_fraction (None) - Fraction of images to be used. The images are selected randomly. If None, all the
                                  images  will be used
        """
        root = env_settings().youtubevis2021_dir if root is None else root
        super().__init__(root, image_loader)
        if split == 'train':
            self.img_pth = os.path.join(root, 'train/JPEGImages/')
            self.anno_path = os.path.join(root, 'annotations/youtube_vis_2021_train.json')
        elif split == 'valid':
            self.img_pth = os.path.join(root, 'valid/JPEGImages/')
            self.anno_path = os.path.join(root, 'annotations/youtube_vis_2021_valid.json')
        else:
...
def main():
    dataset = YoutubeVIS2021("train", root="/home/tiger/ws/tracking_data/Youtube-VIS-2021")
    ...
    #     print(v_str)
    # print(dir(dataset.coco_set))

    # seq = dataset.sequence_list[0]
    # rle = dataset.coco_set.anns[seq['anno_ids'][0]]
    # mask = dataset.coco_set.annToMask(rle)
    # compressed_rle = mask_utils.frPyObjects(rle['segmentation'], rle['segmentation']['size'][0], rle['segmentation']['size'][1])
    # mask2 = mask_utils.decode(compressed_rle)
    ...
    #             cnt += 1
        ...
        # print(k, v['id'], v['video_id'], v['image_id'], v['instance_id'])
